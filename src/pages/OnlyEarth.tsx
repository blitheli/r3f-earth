...
// TypeScript 的模块声明扩展，用于让 @react-three/fiber 支持自定义 JSX 元素 atmosphereLight：
// 告诉 TypeScript，JSX 里可以写 <atmosphereLight />，并自动推断其 props 类型。
...
function Content() {
  ...
  /*
  动态更新 WebGPU 渲染器的 contextNode，使其包含 getAtmosphere 方法，返回当前的大气上下文对象 atmosphereContext。
    renderer.contextNode = context({...})：将渲染器的 contextNode 替换为新的 context 节点对象。
    ...renderer.contextNode.value：保留原有 contextNode 的属性。
    getAtmosphere: () => atmosphereContext：添加或覆盖 getAtmosphere 方法，返回最新的大气上下文。
  useLayoutEffect 保证在 React 渲染后、DOM 更新前执行，确保 contextNode 始终与 atmosphereContext 同步。
  */
  useLayoutEffect(() => {
  ...
  return (
    <WebGPUCanvas
      ...
      renderer={{
        ...
        // 在渲染器初始化后，执行一个回调函数，添加自定义的光照节点到渲染器的节点库中。
        onInit: (renderer) => {
